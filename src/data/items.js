...
export const ITEMS = {
  ...
  // KHÔNG BÁN ĐƯỢC (value 0) — cả Mảnh lẫn 11 Đồ Phổ Bộ dưới đây. Nút Bán trong popup vật phẩm chỉ
  // hiện khi `value > 0` nên để 0 là tắt hẳn đường bán, KHÔNG phải vá sellItem.
  // Vì sao bắt buộc: (1) setUnlocked() chỉ đọc `countItem(blueprintId) > 0`, bán tờ Đồ Phổ đi là
  // KHOÁ LẠI cả bộ đã ghép xong; (2) trọn bộ tốn 420 Mảnh ~ một tháng rưỡi cày, mà nút "Bán hết"
  // không có bước xác nhận — một cú bấm nhầm là bay sạch, đổi lấy số bạc bằng hơn chục lượt Thái Hư.
  manhTrangBi: { id: 'manhTrangBi', name: 'Mảnh Trang Bị Hoàng Kim', icon: '🔆', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Kim loại quý ngưng từ tà khí Yêu Vương và tầng sâu Thái Hư, lóe hoàng kim lạnh. Chất liệu chung để ghép trang bị của các Bộ Trang trấn thân.' },
  dpset_kimQuang: { id: 'dpset_kimQuang', name: 'Đồ Phổ Bộ Kim Quang', icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Bản đồ phổ khắc bí pháp luyện Bộ Kim Quang — bạch kim hộ thân. Lĩnh hội rồi mới có thể dùng Mảnh Trang Bị Hoàng Kim ghép nên từng món của bộ.' },
  // --- Đồ Phổ 10 BỘ BẠCH KIM (đợt 2). Rơi ở `loot.rare` của Bí Cảnh "nhà" từng bộ, ghép theo HỆ
  //     (data/dungeon.js) — 10%/lượt thông quan, engine bỏ qua bản đã có. Tên phó bản để đối chiếu
  //     nằm ở BK_HOME trong data/gear.js.
  dpset_anBang:    { id: 'dpset_anBang',    name: 'Đồ Phổ Bộ An Bang',    icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Quyển đồ phổ nặng như đá núi, giở ra nghe tiếng chuông trấn. Ghi phép luyện giáp trụ của kẻ đứng chắn cho một phương bờ cõi.' },
  dpset_nhuTinh:   { id: 'dpset_nhuTinh',   name: 'Đồ Phổ Bộ Nhu Tình',   icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Lụa mỏng thêu chỉ bạc, chữ mềm như nét nước. Chép phép luyện y giáp của nữ hiệp lấy nhu thắng cương, thương mà không tàn.' },
  dpset_bachHong:  { id: 'dpset_bachHong',  name: 'Đồ Phổ Bộ Bạch Hồng',  icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Quyển trục trắng vắt một vệt mực chéo như cầu vồng xuyên trời. Ghi phép luyện trang bị dành cho kẻ một kiếm phân sinh tử.' },
  dpset_thuongLan: { id: 'dpset_thuongLan', name: 'Đồ Phổ Bộ Thương Lan', icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Da giao long ép thành quyển, sờ vào lạnh buốt tận xương. Chép phép luyện giáp hàn khí, nước không thấm lửa không bén.' },
  dpset_dinhQuoc:  { id: 'dpset_dinhQuoc',  name: 'Đồ Phổ Bộ Định Quốc',  icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Bản khắc trên phiến thiết dày, mép còn vết đao chém. Ghi phép luyện giáp nặng của tướng soái đạp trận bình cương.' },
  dpset_thanhHu:   { id: 'dpset_thanhHu',   name: 'Đồ Phổ Bộ Thanh Hư',   icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Quyển giấy dó nhẹ như mây, cầm lâu tưởng sắp bay khỏi tay. Chép phép luyện đạo bào ngưng thần, gió qua tay áo mà tay áo không động.' },
  dpset_hongAnh:   { id: 'dpset_hongAnh',   name: 'Đồ Phổ Bộ Hồng Ảnh',   icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Quyển bọc vải đỏ sẫm, chữ chỉ hiện dưới ánh trăng. Ghi phép luyện dạ hành y của kẻ đi trong bóng, giết xong mới có người thấy.' },
  dpset_tuDien:    { id: 'dpset_tuDien',    name: 'Đồ Phổ Bộ Tử Điện',    icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Mặt quyển còn rịn mùi khét, nét chữ nứt hình tia chớp. Chép phép luyện chiến giáp lôi đình, chớp đi trước tiếng sấm theo sau.' },
  dpset_thatSat:   { id: 'dpset_thatSat',   name: 'Đồ Phổ Bộ Thất Sát',   icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Quyển da thuộc thấm màu huyết dụ, mở ra sát khí xộc lên mũi. Ghi phép luyện giáp trụ dưới sao Thất Sát, đã ra đòn là không có đường lui.' },
  dpset_minhVuong: { id: 'dpset_minhVuong', name: 'Đồ Phổ Bộ Minh Vương', icon: '📜', type: 'khac', quality: 'coBan', value: 0, boss: true, desc: 'Kinh quyển vàng sẫm in dấu ấn kim cang, đặt xuống là không nhấc lên nổi. Chép phép luyện giáp bất hoại của Minh Vương phẫn nộ.' },
