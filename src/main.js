...
const gameStore = {
  ...
  tmSpeedMul(d) { void this._tick; return disciTocMul(this.tm, d); },   // 1 NGUỒN — engine/tongmon.disciTocMul
  ...
  // Đạo Tâm 1 đệ tử -> thanh ẩn: tên bậc + màu + vị trí kim trên trục (0..100%). KHÔNG trả con số thô.
  // Tâm tình 1 đệ tử -> tên bậc + màu + vị trí kim trên trục (0..100%).
  tmTamTinh(d) {
    void this._tick;
    const v = tamTinhOf(d), tier = tamTinhTier(v);
    return { ten: tier.name, color: tier.color, pct: Math.round((v + TAM_TINH_BIEN) / (TAM_TINH_BIEN * 2) * 100) };
  },
  // Nhật ký đệ tử, mới nhất trước. `khi` = chữ thời gian đã dựng sẵn (markup khỏi tự tính).
  tmNhatKy(d) {
    void this._tick;
    return ((d && d.nhatKy) || []).map((n) => ({ khi: this.notifAgo(n.t), su: n.su || '', txt: n.txt || '' }));
  },
  ...
  // Sổ Danh Khí đã thức tỉnh, mới nhất trước — vẽ ở Tổ Sư Điện.
  get tmDanhKhiLuc() {
    void this._tick;
    return danhKhiDaThuc(this.tm).map((x) => ({
      uid: x.uid, ten: x.ten, tieuSu: x.tieuSu || '',
      doi: (x.doi || []).join(' → '),
      tenDo: (this.ITEMS[x.gearId] || {}).name || '',
      icon: this.ico(x.gearId, (this.ITEMS[x.gearId] || {}).icon),
    }));
  },
  tmDaoTam(d) {
    void this._tick;
    const v = daoTamOf(d), tier = daoTamTier(v);
    return { ten: tier.name, color: tier.color, pct: Math.round((v + DAO_TAM_BIEN) / (DAO_TAM_BIEN * 2) * 100) };
  },
  get tmLuanVoData() {
    void this._tick;
  ...
  lbHSXoa(r) {
    ...
    this.hoiXacNhan({
      ...
      nut: 'Gỡ', nguy: true,
      xong: async () => {
        const rr = await cloudHeSoDong(r.id);
        if (!rr.ok) { this.showToast('Không gỡ được — ' + rr.reason); return; }
  ...
  devTmBanGiaBao(n) {   // ban Gia Bảo ngẫu nhiên cho MỌI đệ tử (dựng cảnh Danh Khí)
    ...
    this.devSave(); this._tick++; this.showToast('Dev: ban ' + dem + ' món Gia Bảo');
  },
  devTmDanhKhiEpThuc() { const n = danhKhiEpThuc(this.state); this.devSave(); this._tick++; this.showToast('Dev: ' + n + ' món thức tỉnh Danh Khí'); },
  ...
  async taiSan() {
    ...
    try {
      ...
      if (this.moChua('sanThuMua')) {
        ...
        const th = await cloudSanTmThuHoi();
        if (th && th.ok && th.don > 0) {
          this.showToast('Đã hoàn ' + this.fmt(th.hoan) + ' Bạc từ đơn thu mua hết hạn.');
          await this._sanNapLai();
        }
        const [c, d, e] = await Promise.all([cloudSanTmDs(80), cloudSanTmCuaToi(50), cloudSanTmSoKhop(30)]);
        this.sanTmDs = (c.ok ? c.ds : []); this.sanTmCuaToi = (d.ok ? d.ds : []);
        this.sanTmSoKhop = (e.ok ? e.ds : []);
      }
  ...
  get sanTmDonCuaLang() {
    ...
    return (this.sanTmDs || []).filter((d) => d && d.nguoi_dat !== me && this.sanTmConMs(d) > 0)
  ...
  /**
   * HẠN ĐƠN 48 GIỜ — số mili giây còn lại. Số âm là đã quá hạn.
   * ⚠ Mốc lấy từ đồng hồ máy người chơi nên chỉ dùng để HIỆN. Hàng rào thật là chốt `don-het-han`
  ...
  sanTmConLai(d) {
    const ms = this.sanTmConMs(d);
    if (ms <= 0) return 'Hết hạn';
    const gio = Math.floor(ms / 3600000);
    return gio >= 1 ? ('Còn ' + gio + ' giờ') : ('Còn ' + Math.max(1, Math.round(ms / 60000)) + ' phút');
  },
  /**
   * SỔ KHỚP của chủ đơn: từng lô hàng đã giao vào đơn của mình, mới nhất trên đầu.
   * ⚠ Bảng đơn chỉ giữ SỐ GỘP (`so_dat` trừ `so_con`), không giữ từng lần khớp. Nguồn của sổ này
   *   là `san_rao`, lọc theo tiền tố `tm:` nên không lẫn sang món mình mua ở Chợ.
   */
  get sanTmSoKhopDs() {
    return (this.sanTmSoKhop || []).map((r) => ({
      id: r.id, ten: (this.ITEMS[r.item_id] || {}).name || r.item_id,
      obj: this.ITEMS[r.item_id] || { id: r.item_id }, so: r.so_luong || 1, gia: r.gia || 0,
      ban: r.ten_ban || '—', luc: this.saveTimeText(r.xong_luc ? new Date(r.xong_luc).getTime() : 0),
    }));
  },
  async sanTmDat() {
