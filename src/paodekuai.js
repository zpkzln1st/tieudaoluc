...
'@media (prefers-reduced-motion:reduce){.pk-turn.on{animation:none}}',
    ...
    '.kh-nho .pk-title{left:10px;top:7px}.kh-nho .pk-title .hz{font-size:18px}.kh-nho .pk-title .vz{font-size:11px}',
    '.kh-nho .pk-chieu{left:11px;top:29px;font-size:9.5px}',
    ...
    '.kh-nho .pk-cur{top:38px;font-size:11px;padding:4px 11px;max-width:76%}',
    // width:auto — nhãn dài hơn ô 46px thì tràn ra hai bên rồi bị mép khung xén mất chữ đầu
    '.kh-nho .pk-left{left:8px;gap:6px;top:64%}.kh-nho .pk-b{width:auto}',
    '.kh-nho .pk-b .ic{width:27px;height:27px}.kh-nho .pk-b .ic svg{width:15px;height:15px}.kh-nho .pk-b span{font-size:8.5px}',
    '.kh-nho .pk-act{bottom:8px;gap:6px}.kh-nho .pk-btn{padding:5px 13px;font-size:11.5px}',
    '.kh-nho .pk-seat{padding:3px 7px 3px 3px;gap:5px}.kh-nho .pk-av{width:26px;height:26px}',
    '.kh-nho .pk-seat .nm{font-size:10.5px}.kh-nho .pk-seat .bh{display:none}.kh-nho .pk-seat .ct{font-size:9px}',
    '.kh-nho .pk-chat,.kh-nho .pk-view{bottom:52px}',
    // mobile
    ...
    '@media (max-width:600px){.pk-root{aspect-ratio:auto;height:86dvh;max-height:none}',
    ...
    '  .pk-left{left:0;right:0;top:auto;bottom:82px;transform:none;flex-direction:row;justify-content:center;gap:14px}',
    '  .pk-b{width:auto}.pk-b .ic{width:33px;height:33px}',
    '  .pk-act{bottom:14px;gap:5px;flex-wrap:wrap;justify-content:center;width:97%}.pk-btn{padding:6px 11px;font-size:11.5px}',
    ...
    '  .pk-toast{font-size:11px;top:68px;left:10px;max-width:calc(100% - 20px)}.pk-chat{bottom:132px}.pk-view{bottom:132px}.pk-view .lb{display:none}',
...
function mountPaoDeKuai(host, opts) {
  ...
  function anchorOf(s) {
    ...
    //   7,5·0,39·2,0 + 0,70 = 6,55 trong khi nửa mặt nỉ chỉ 5,41. Siết bước còn 0,31 thì bó rộng
    //   5,35 ≈ mặt nỉ ⇒ ràng buộc chuyển về mặt nỉ, CẢ BÀN LẪN BÀI to thêm ~1/5.
    if (W() / H() < 1) return { pos: A.pos, rotY: A.rotY, tilt: A.tilt, arc: 0, step: 0.31, scale: 2.0 };
    // Khung THẤP (điện thoại nằm ngang / toàn màn hình): user kêu lá bài mình bé.
    // ⚠ Tăng `scale` suông là VÔ ÍCH: bó 16 lá rộng 12.88 > mặt nỉ 11.1 nên chính nó đang ghìm
    //   khung ngắm — lá to lên thì camera lùi ra đúng bằng ấy. Phải cho CHỒNG SÂU hơn để tổng
    //   bề ngang không tăng: step 0.41→0.355 · scale 1.88→2.10 ⇒ lá to thêm ~1/9, bó lại hẹp hơn.
    //   Lộ 51% mặt lá, mà bậc + chất nằm gọn trong góc trên-trái (~27%) nên vẫn đọc trọn.
    if (H() < 460) return { pos: A.pos, rotY: A.rotY, tilt: A.tilt, arc: 0, step: 0.355, scale: 2.10 };
    return A;
  }
  ...
  function onResize() {
    ...
    if ($('.pk-banner').classList.contains('show')) vuaKhung($('.pk-end'), root);
    renderer.setSize(W(), H());
    ...
    chotHopBai();                      // camera doi => hop bai chieu ra man doi => do lai
  }
  ...
  /** Hộp bài lúc ĐỦ LÁ — chốt một lần sau khi chia; bài chỉ vơi đi nên né hộp này là né cả ván. */
  var hopDay = [];
  function chotHopBai() { for (var s = 0; s < 3; s++) hopDay[s] = hopBai(s); }
  ...
  function datSeat() {
    ...
    for (var s = 1; s < 3; s++) {
      ...
      if (hep) {
        ...
        // ⚠ Tỉ lệ cứng đoán không nổi (đo được: 412×915 thẻ còn đè 431 và 774 px² lên cột bài).
        //   ĐO hộp bài — chốt lúc đủ lá — rồi treo thẻ lên TRÊN nó; vẫn đứng yên vì hộp đã chốt.
        //   Né cả hộp bài NHÀ MÌNH nữa: bàn ba người, thẻ hai bên nằm ngay trên dải bài dưới cùng.
        if (hopDay[s]) py = Math.min(py, hopDay[s].y0 - h / 2 - 14);
        if (hopDay[0]) py = Math.min(py, hopDay[0].y0 - h / 2 - 14);
      } else {
  ...
  function ketVan() {
    ...
    vuaKhung($('.pk-end'), root);      // ép bảng vừa khung, khỏi phải lăn chuột xem kết quả
    var bet = -1, xau = 1e9;
  ...
  try {
    ...
    window.__pdk = {
      scene: scene, camera: camera, handGroups: handGroups, hands: hands, MAT: MAT, GEO: GEO,
      root: root, seatEls: seatEls,
...
function injectSanhStyle() {
  ...
  st.textContent = [
    ...
    // Khổ điện thoại: xem chú thích cùng chỗ ở tienlen.js — rút một chiếu từ ~250px còn ~100px.
    '@media (max-width:640px){.pks-chieu{gap:10px;padding:10px}.pks-quat{width:80px;height:62px}',
    '  .pks-quat img{width:37px;height:48px}.pks-quat img:nth-child(1){top:9px}.pks-quat img:nth-child(2){left:21px;top:4px}.pks-quat img:nth-child(3){left:42px;top:9px}',
    '  .pks-tn{font-size:14px}',
    '  .pks-lo{font-size:10.5px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    '  .pks-ds{font-size:10.5px;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    '  .pks-mt{gap:6px;margin-top:6px}.pks-tag{font-size:10px;padding:2px 8px}}',
  ].join('\n');
  document.head.appendChild(st);
}
